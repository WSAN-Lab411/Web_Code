...
while(true)
{
	$sql1="SELECT bt FROM bantin_map WHERE bt LIKE '%#OK:0000000F%' LIMIT 1";
	$query1 = mysql_query($sql1);
	$row_no = mysql_num_rows($query1);
	if($row_no != 0)
	{
		$bt = "Tất cả các van đã được tắt thành công.";
		break;
	}
	$now1 = getdate();
...
/*
	for ($i = 0; $i<5; $i++){
		...
		if($row_no==0){
			...
			while ($row1 = mysql_fetch_array($query1)){
				$bt = "Tất cả các van đã tắt thành công!";	
	...
	*/
$sql1="DELETE FROM bantin_map WHERE bt LIKE '%#OK:0000000F%' ";
$query1 = mysql_query($sql1);
